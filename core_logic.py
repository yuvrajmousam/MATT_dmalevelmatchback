...
def update_ads_with_factors(master_spec_bytes, ads_bytes, ads_is_csv, factor_rows, granular_df, lower, upper):
    ...
    """
    # 1. Load Master Spec
    master_df = pd.read_excel(BytesIO(master_spec_bytes), sheet_name="Model Specifications", dtype=str, engine='openpyxl')
    
    # Identify Master Cols (Variable, Include, PMF) using basic keyword matching
    ...
    var_col = find_col(master_df, ["variable"])
    inc_col = find_col(master_df, ["include"])
    pmf_col_header = find_col(master_df, ["post", "pmf"])
    
    if not all([var_col, inc_col, pmf_col_header]):
        raise ValueError("Could not find Variable, Include, or Post Multiplication columns in Master Spec.")
    ...
    # Find ADS cols
    ads_geo_col = find_col(df_ads, ["geo", "geography"])
    ads_time_col = find_col(df_ads, ["quarter", "time"])
    ...
    output_ads = BytesIO()
    if ads_is_csv:
        df_ads.to_csv(output_ads, index=False)
    else:
